...
<script lang="ts">
	...
	const blockTypes: ContentBlockType[] = ['heading', 'paragraph', 'button', 'divider'];
	...
	let blocks: ContentBlock[] = [
		...
		{ id: createBlockId(), type: 'button', text: 'View your pass', url: 'https://example.org/pass' }
	];
	...
	function parseCsvInput(nextCsvText: string): void {
		...
		try {
			...
			attendees = [];
			csvError = error instanceof Error ? error.message : 'Unable to parse CSV.';
		}
	}
	...
	function updateBlock(blockId: string, key: keyof ContentBlock, value: string | ContentBlockType): void {
		blocks = blocks.map((block) => (block.id === blockId ? { ...block, [key]: value } : block));
	}
	...
	function addBlock(type: ContentBlockType): void {
		blocks = [
			...blocks,
			{
				id: createBlockId(),
				type,
				text: type === 'divider' ? '' : type === 'button' ? 'Click here' : 'New content',
...
<main>
	<section class="panel">
		...
		<p class="hint">Upload attendees as CSV, build blocks, preview personalization, and send with Gmail API.</p>

		<form on:submit={sendEmails}>
			<div class="group">
				<label for="csv-file">Upload attendee CSV</label>
				<input id="csv-file" type="file" accept=".csv,text/csv" on:change={onCsvFileUpload} />
				<textarea
					rows="8"
					placeholder="email,first_name,last_name"
					value={csvText}
					on:input={(event) => parseCsvInput((event.currentTarget as HTMLTextAreaElement).value)}
				></textarea>
				{#if csvError}
			...
			<div class="group split">
				...
				<div>
					...
					<input id="from-email" bind:value={fromEmail} placeholder="organiser@example.org" />
				</div>
			</div>
			...
			<div class="group">
				...
				<div class="tokens">
					{#each placeholderTokens as token}
						<button type="button" on:click={() => appendSubjectToken(token)}>{`{{${token}}}`}</button>
					{/each}
				</div>
			</div>
			...
			<div class="group">
				...
				<div class="builder-actions">
					{#each blockTypes as type}
						<button type="button" on:click={() => addBlock(type)}>Add {type}</button>
					{/each}
				</div>
				...
				{#each blocks as block, index (block.id)}
					<div class="block-card">
						<div class="block-header">
							...
							<button type="button" on:click={() => removeBlock(block.id)}>Remove</button>
						</div>
						<select
							value={block.type}
							on:change={(event) =>
								updateBlock(block.id, 'type', (event.currentTarget as HTMLSelectElement).value as ContentBlockType)}
						>
							{#each blockTypes as type}
						...
						{#if block.type !== 'divider'}
							<input
								...
								value={block.text}
								on:input={(event) =>
									updateBlock(block.id, 'text', (event.currentTarget as HTMLInputElement).value)}
							/>
							<div class="tokens">
								{#each placeholderTokens as token}
									<button type="button" on:click={() => appendBlockToken(block.id, token)}>{`{{${token}}}`}</button>
								{/each}
							</div>
						{/if}
						...
						{#if block.type === 'button'}
							<input
								...
								value={block.url}
								on:input={(event) =>
									updateBlock(block.id, 'url', (event.currentTarget as HTMLInputElement).value)}
							/>
						{/if}
					</div>
				{/each}
			...
			<button class="send" type="submit" disabled={isSending}>{isSending ? 'Sending...' : 'Send emails'}</button>
		</form>
		...
		{/if}
		{#if sendResponse}
			<p class="ok">Sent {sendResponse.sentCount} of {sendResponse.totalRecipients} emails.</p>
		...
		{#if attendees.length > 0}
			...
			<select id="preview-attendee" bind:value={selectedAttendee}>
				{#each attendees as attendee, index}
					<option value={index}>{attendee.first_name} {attendee.last_name} ({attendee.email})</option>
				{/each}
			</select>
		{/if}
